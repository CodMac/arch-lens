...
**强制执行协议快照**：Agent 在扫描前必须从 [官方文档库](https://github.com/CodMac/arch-lens/tree/main/doc) 抓取全部协议副本并落盘至本地 `~/.archlens/protocols_snapshot/{缺陷名称}.md`。扫描过程必须以此本地快照为准。
...
* **协议快照**：同步远程协议至 `~/.archlens/protocols_snapshot/{缺陷名称}.md`，优先从 `{skill目录}/references/` 中获取，本地不存在再从 官方文档库 同步。
* **原子扫描**：**严禁混合扫描**。Agent 必须按协议编号逐个触发，每项缺陷生成独立的中间结果文件（如 `~/.archlens/task_{扫描文件清单MD5}/tmp_smell_{缺陷序号}.json`），过程中使用的指标计算脚本，也要输出落盘 。
* **上下文清理**：每项原子扫描结束后，必须显式声明已清空内存中的度量指标堆栈，确保下一项扫描的独立性。

### 第二步：诊断 (Diagnosis) —— 逻辑对齐
* **指标对撞**：AI 读取前一步的本地中间结果文件（如 `~/.archlens/task_{扫描文件清单MD5}/tmp_smell_{缺陷序号}.json`）），并与 `~/.archlens/protocols_snapshot/{缺陷名称}.md` 中的判定阈值进行文本对撞。
* **汇聚落盘**：将所有原子 JSON 结果汇聚为一份统一的《arch_report.md》，并按协议定义的严重程度排序。
